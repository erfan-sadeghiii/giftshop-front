...
export default function AmazingSlider({isAdmin=false}) {
    ...
    const fetchSlider = async () => {
      try {
        const { data } = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/shop/amazing-slider/`
        );

        if (!data.length) return;

        const firstSlider = data[0];
        setSlider(firstSlider);

        const sorted = firstSlider.products
  ...
  const slides = products; // ✅ define slides
  if(slider){
    return (
